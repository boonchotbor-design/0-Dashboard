...
let newGrid = '<div class="main-grid" style="display:flex; align-items:flex-start;">\n';
newGrid += '      <div style="flex:1; display:flex; flex-direction:column; gap:20px; min-width:0;">\n';
newGrid += '        ' + milestone + '\n\n';
newGrid += '        ' + docOwner + '\n\n';
newGrid += '        ' + overall + '\n';
newGrid += '      </div>\n';
...
const endGrid = finalH.indexOf('<script>');
if (endGrid !== -1) {
    const lastDiv = finalH.lastIndexOf('</div>', endGrid - 20);
    const patchedH = finalH.substring(0, lastDiv) + '      </div>\n    </div>\n  </div>' + finalH.substring(endGrid - 10);
